...
CREATE TABLE IF NOT EXISTS t_group_membership
(
  id uuid NOT NULL,
  created_at timestamp(6) without time zone NOT NULL,
  ...
  CONSTRAINT fk_group_membership__to__group FOREIGN KEY (group_id)
    REFERENCES t_group (id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE CASCADE,
  CONSTRAINT fk_group_membership__to__user FOREIGN KEY (user_id)
